...
export default function AnimeModal() {
  ...
  useEffect(() => {
    ...
    const onEdit = (e) => {
      ...
      setForm({
        ...
        cover: anime.cover || "",
        genres: Array.isArray(anime.genres) ? anime.genres.join(", ") : "",
        year:
  ...
  const selectResult = (r) => {
    ...
    setForm((f) => ({
      ...
      genres: (r.genres || []).filter((g) => g !== "Award Winning").join(", "),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isCompleted) {
        ...
        if (!isNaN(y) && !isNaN(m) && m >= 1 && m <= 12) {
          ...
          if (
            ...
            const resolved = await resolveActualFinishDate(year, month);
            if (resolved === null) {
              setSubmitting(false);
              return;
            }
      ...
      if (editingSnapshot) {
        ...
        setAnimeData((prev) =>
          prev.map((a) => (a.id === updatedAnime.id ? updatedAnime : a)),
        );
        ...
        <form onSubmit={handleSubmit} id="addAnimeForm">
          <div className="form-group form-group-search">
            ...
            <input
              ...
              placeholder={
                isEditing ? "Title cannot be changed" : "Search for an anime..."
              }
              required
              disabled={isEditing}
              autoComplete="off"
            ...
            {searching && (
              <div className="search-loading">
                <i className="fas fa-spinner fa-spin" /> Searching AniList...
              </div>
            )}
          ...
          <div className="form-row form-row-3">
            ...
            <div className="form-group">
              <label htmlFor="animeDuration">Duration (min)</label>
              <input
                id="animeDuration"
                type="number"
                min={1}
                ...
                title={durationEditable ? "" : "Auto-set for non-movie types"}
              />
            </div>
          </div>
          ...
          <div className="form-row form-row-2 completion-date-box">
            <div className="form-group">
              <label htmlFor="animeYear">
                <i className="fas fa-calendar-alt" /> Completion Year
              </label>
              <select
                id="animeYear"
                value={form.year}
            ...
            <div className="form-group">
              <label htmlFor="animeMonth">
                <i className="fas fa-calendar" /> Completion Month
              </label>
              <select
                id="animeMonth"
                value={form.month}
          ...
          <div className="form-actions">
            ...
            >
              {submitting ? (
                ...
                <>
                  <i className={`fas ${isEditing ? "fa-save" : "fa-plus"}`} />{" "}
