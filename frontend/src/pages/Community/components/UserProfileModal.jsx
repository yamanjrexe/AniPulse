...
function timestampOf(a) {
  return (
    a?.updatedAt || a?.createdAt || a?.actualFinishDate || a?.finishDate || ""
  );
}

function sortRecentFirst(list) {
  return [...list].sort((a, b) => {
    const da = timestampOf(a);
    const db = timestampOf(b);
...
function buildLocalProfile(user) {
  ...
  const recentActivity = (() => {
    try {
      return JSON.parse(localStorage.getItem("activityLog") || "[]").slice(
        0,
        15,
      );
    } catch {
  ...
  return {
    ...
    animeList: {
      completed: sortRecentFirst(completed),
      watching: sortRecentFirst(watching),
      planToWatch: sortRecentFirst(plan),
    },
    achievements,
    recentActivity,
    isCurrentUser: true,
...
function normalizeServerProfile(data) {
  ...
  const animeList = {
    completed: sortRecentFirst(data.animeList?.completed || []),
    watching: sortRecentFirst(data.animeList?.watching || []),
    planToWatch: sortRecentFirst(
...
export default function UserProfileModal({ userId, onClose }) {
  ...
  // Build a title lookup from every anime we have access to on this
  // profile, so favorite anime IDs resolve to real titles.
  const animeTitleMap = useMemo(() => {
    const map = {};
    if (!profile?.animeList) return map;
    ["completed", "watching", "planToWatch"].forEach((key) => {
      (profile.animeList[key] || []).forEach((a) => {
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <i className="fas fa-user-circle" style={{ color: "#FCE706" }} />
            User profile
            {isSelf && (
              <span
                style={{
          ...
          <div className="profile-modal-body" style={{ padding: 20 }}>
            <div
              style={{
                ...
                backgroundColor: profile.cover ? "transparent" : "#171717",
                display: "flex",
                gap: 16,
                alignItems: "center",
                position: "relative",
              ...
              <img
                src={
                  ...
                  `https://ui-avatars.com/api/?name=${encodeURIComponent(
                    profile.name || "User",
                  )}&background=FCE706&color=050505&bold=true&size=200`
                }
                alt={profile.name}
                style={{
                  width: 84,
                ...
                onError={(e) => {
                  e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(
                    profile.name || "User",
                  )}&background=FCE706&color=050505`;
                }}
              />
              <div
                style={{
                ...
                {!profile.isCurrentUser && (
                  <button
                    ...
                    style={{
                      marginTop: 10,
                      padding: "6px 14px",
                      background: profile.isFriend ? "#292929" : "#FCE706",
                      color: profile.isFriend ? "#797979" : "#050505",
                      border:
                        "1px solid " +
                        (profile.isFriend ? "#3A3A3A" : "#FCE706"),
                      borderRadius: 8,
                      cursor: profile.isFriend ? "not-allowed" : "pointer",
                      fontWeight: 600,
                      fontSize: "0.8rem",
                      fontFamily: "inherit",
                    }}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {profile.favoriteAnime?.length > 0 && (
                ...
                >
                  {profile.favoriteAnime.slice(0, 6).map((fav, i) => {
                    const id = typeof fav === "object" ? fav.id : fav;
                    const fallbackTitle =
                      typeof fav === "object" ? fav.title : null;
                    const title =
                      fallbackTitle || animeTitleMap[id] || `Anime #${id}`;
                    return (
                      <span
                        key={i}
                        style={{
            ...
            <div style={{ minHeight: 200 }}>
              {(tab === "completed" || tab === "watching" || tab === "plan") &&
                (() => {
                  const key = tab === "plan" ? "planToWatch" : tab;
                  ...
                  return (
                    ...
                    >
                      {ACHIEVEMENTS.map((ach) => {
                        const isUnlocked = list.includes(ach.id);
                        return (
                          <div
                            key={ach.id}
                            title={ach.desc}
                          ...
                          >
                            <div
                              style={{
                                fontSize: 18,
                                color: isUnlocked ? "#FCE706" : "#555555",
                                width: 28,
                                textAlign: "center",
                                flexShrink: 0,
                              }}
                            ...
                            >
                              <div
                                style={{
                                  fontSize: "0.75rem",
                                  fontWeight: 600,
                                  color: isUnlocked ? "#FBFBFB" : "#797979",
                                }}
                              >
                                {ach.title}
                              </div>
